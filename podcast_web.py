...
def show_new_podcast_page():
    st.title("🎙️ TulgaBOT Podcast")
    if not st.session_state.show_chat_view:
        with st.expander("⚙️ Подкаст баптаулары", expanded=st.session_state.settings_expanded):
            col1, col2 = st.columns(2)
            with col1:
                char1_name = st.selectbox(
                    "Бірінші кейіпкер",
                    options=["Кейіпкерді таңдаңыз"] + [name for name in CHAR_ID.keys() if not name.startswith('#')],
                    key="char1"
                )
                if char1_name == "Кейіпкерді таңдаңыз":
                    char1_name = None
            with col2:
                ...
                if char2_name == "Кейіпкерді таңдаңыз":
                    char2_name = None
            topic = st.text_input("Подкаст тақырыбы") 
            exchanges = st.number_input("Алмасулар саны", min_value=1, max_value=10, value=3)
            start_disabled = (
                st.session_state.is_podcast_running or
                not char1_name or not char2_name or char1_name == char2_name or not topic
            )
            if st.button("🎙️ Подкастты бастау", disabled=start_disabled):
    ...
    if st.session_state.show_chat_view or st.session_state.is_podcast_running:
        if st.session_state.is_podcast_running:
            st.info(f"🎙️ Подкаст жасалуда... {st.session_state.current_chars['char1']} мен {st.session_state.current_chars['char2']} сөйлесуде") 
            time.sleep(2)
            st.rerun()
        if st.session_state.podcast_history:
            ...
            if not st.session_state.is_podcast_running:
                if st.button("🎙️ Жаңа подкаст жасау", key="new_podcast_after_completion"):
                    st.session_state.show_chat_view = False
                    st.session_state.settings_expanded = True
                    st.session_state.podcast_history = []
                    st.rerun()
    ...
    if os.path.exists(TEMP_FILE):
        try:
            ...
            if result.get("completed", False):
                if "error" in result:
                    st.error(f"❌ Подкаст жасау кезінде қате орын алды: {result['error']}")
                    st.session_state.is_podcast_running = False
                    st.session_state.show_chat_view = False
                else:
                    ...
                    st.session_state.is_podcast_running = False
                    st.success("✅ Подкаст сәтті аяқталды!")
                os.remove(TEMP_FILE)
                st.rerun()
        except Exception as e:
            st.error(f"❌ Подкаст нәтижесін оқу кезінде қате орын алды: {str(e)}")
            st.session_state.is_podcast_running = False

    with st.sidebar:
        if st.button("➕ Жаңа подкаст", key="new_chat_btn"):
            st.session_state.selected_podcast = None
            st.session_state.show_chat_view = False
            st.session_state.settings_expanded = True
            st.rerun()
        ...
        if not st.session_state.all_podcasts:
            st.info("📭 Әзірше подкаст тарихы бос")
        else:
            for idx, podcast in enumerate(st.session_state.all_podcasts):
                topic = podcast.get('topic', 'Тақырып жоқ')
